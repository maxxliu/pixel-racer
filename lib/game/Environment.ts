...
export class Environment {
  ...
  constructor(private readonly spline: TrackSpline, quality: 'low' | 'medium' | 'high') {
    ...
    this.clouds = this.buildClouds(quality === 'low' ? 6 : 12);
    this.buildGround();
    this.buildMountains();
  ...
  private buildSun(): THREE.Mesh {
    ...
    sun.renderOrder = -9;
    this.group.add(sun);
    this.disposables.push(geo, mat, glowGeo, glowMat);
    return sun;
  }

  private buildClouds(count: number): THREE.Group {
    ...
    this.group.add(g);
    this.disposables.push(mat, puff);
    return g;
  }
