  ...
  return (
    <article className="glass flex flex-col overflow-hidden transition-transform hover:-translate-y-1">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <p className="text-xs text-muted">by {track.author}</p>
        <dl className="mt-3 grid grid-cols-3 gap-2 text-xs">
          <div><dt className="text-muted">Length</dt><dd className="hud-num">{formatTrackLength(track.lengthM)}</dd></div>
          <div><dt className="text-muted">Turns</dt><dd className="hud-num">{track.turnCount}</dd></div>
          <div><dt className="text-muted">Plays</dt><dd className="hud-num">{track.plays}</dd></div>
        </dl>
        <div className="mt-4 flex gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={onDelete} aria-label={`Delete ${track.name}`}>✕</Button>
        </div>
      </div>
    </article>
  );
